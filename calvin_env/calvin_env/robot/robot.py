    ...
    def apply_action(self, action):
        ...
        if isinstance(action, dict):
            if action["type"] == "joint_rel":
                current_joint_states = np.array(
                    list(zip(*p.getJointStates(self.robot_uid, self.arm_joint_ids)))[0]
                )
                ...
                target_ee_pos, target_ee_orn, self.gripper_action = (
                    self.relative_to_absolute(action["action"])
                )
